...
mrp_error_LMO = np.linalg.norm(np.array([mrp_BRc(mrp_LMO[i], t_LMO[i]) for i in range(len(t_LMO))]), axis=1)
...
Path("output/task_10").mkdir(parents=True, exist_ok=True)
create_submission_txt("output/task_10/mrp_15.txt", mrp_LMO[np.where(t_LMO == 15)[0][0]])
create_submission_txt("output/task_10/mrp_100.txt", mrp_LMO[np.where(t_LMO == 100)[0][0]])
create_submission_txt("output/task_10/mrp_200.txt", mrp_LMO[np.where(t_LMO == 200)[0][0]])
create_submission_txt("output/task_10/mrp_400.txt", mrp_LMO[np.where(t_LMO == 400)[0][0]])
